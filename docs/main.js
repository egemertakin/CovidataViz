document.addEventListener('DOMContentLoaded', function() {
  ...
  function goToSlide(index) {
    ...
    if (slides[currentSlide].id === 'world-insight') {
      ...
      animateSequential();
    } else if (slides[currentSlide].id === 'data-sources') {
      // No specific animation needed, CSS handles it
    }
  ...
  // Prevent default wheel behavior except in data sources slide
  document.addEventListener('wheel', (e) => {
    // Allow scrolling in iframes
    if (e.target.closest('iframe')) return;
    
    // Allow scrolling in the data sources slide content
    if (currentSlide === getSlideIndex('data-sources') && 
        (e.target.closest('.slide-content') || e.target.closest('.sources-container'))) {
      return; // Let the default scroll behavior happen
    }
    
    // Prevent scroll on main content for other slides
    e.preventDefault();
    
    // No slide navigation with wheel - removed this functionality
  }, { passive: false });
  
  // Helper function to get slide index by ID
  function getSlideIndex(slideId) {
    return Array.from(slides).findIndex(slide => slide.id === slideId);
  }
  
  // Touch navigation - disable swipe for slides except in data sources
  document.addEventListener('touchmove', (e) => {
    if (e.target.closest('iframe')) return;
    
    // Allow touch scrolling in data sources slide
    if (currentSlide === getSlideIndex('data-sources') &&
        (e.target.closest('.slide-content') || e.target.closest('.sources-container'))) {
      return;
    }
  ...
  // Add touch events for navigation arrows
